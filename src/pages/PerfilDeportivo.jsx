...
// ── POP FOTO ──
function PopFoto({ onAdd, onClose }) {
        ...
        <div className="rounded-xl relative overflow-visible"
          ...
          <div className="flex gap-8 items-start">
            <div className="flex flex-col gap-4" style={{ minWidth: '220px' }}>
              ...
              <img src={robotJugador} alt="Robot" className="w-24 object-contain mt-2" />
            </div>
            ...
            <div className="flex-1 flex flex-col gap-3">
              ...
              >
                {preview ? (
                    <img src={preview} alt="Preview" className="h-full w-auto object-contain rounded" />
                ) : (
...
// ── POP CONFIRMACIÓN (igual al de Register) ──
function PopConfirmacion({ bgColor, onContinuar }) {
  return (
      <div className="fixed inset-0 z-50 flex items-center justify-center"
           style={{ background: 'rgba(0,0,0,0.5)' }}>
        <div className="bg-[#e8e8e8] rounded-xl p-8 max-w-md w-full mx-4 flex flex-col items-center text-center">
          ...
          <h2 className="text-xl uppercase leading-tight mb-6"
              style={{ fontFamily: "'Anton SC', sans-serif", color: bgColor }}>
...
export default function PerfilDeportivo() {
  ...
  // Todos los campos obligatorios para habilitar el botón Continuar
  const todoCompleto = posicion && dorsal && pieDominante;

  const handleContinuar = () => {
    const newErrors = {};
    if (!posicion)      newErrors.posicion = 'Selecciona tu posición';
    if (!dorsal)        newErrors.dorsal   = 'Selecciona tu dorsal';
    if (!pieDominante)  newErrors.pie      = 'Selecciona tu pie dominante';
    setErrors(newErrors);
        ...
        <div className="relative z-10 flex-1 px-8 md:px-12 py-6 flex flex-col">
          ...
          <div className="flex flex-wrap items-center gap-6 mb-8">
            ...
            <div className="flex flex-col gap-1">
              <div className="relative">
                <button
                    ...
                    style={{
                      background: '#001a3a',
                      border: errors.posicion ? '2px solid #FF6B6B' : '1px solid rgba(255,255,255,0.3)',
                      fontFamily: "'Poppins', sans-serif"
                    }}>
                <span style={{ color: posicion ? 'white' : errors.posicion ? '#FF6B6B' : 'rgba(255,255,255,0.6)' }}>
                  {posicion || 'Cual es tu posicion'}
                </span>
                ...
                {showPosicion && (
                    <div className="absolute top-full left-0 w-full rounded mt-1 z-30 max-h-48 overflow-y-auto"
                      ...
                      {posiciones.map(p => (
                          <button key={p} onClick={() => { setPosicion(p); setShowPosicion(false); setErrors(prev => ({ ...prev, posicion: '' })); }}
              ...
              {errors.posicion && (
                  <span className="text-red-400 text-xs" style={{ fontFamily: "'Inter', sans-serif" }}>
                {errors.posicion}
              </span>
              )}
            </div>
            ...
            <div className="relative">
              ...
              <span style={{ color: otraPosicion ? 'white' : 'rgba(255,255,255,0.6)' }}>
                {otraPosicion || 'Otra Posicion'}
              </span>
          ...
          <div className="flex items-start gap-10">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex flex-col gap-1 w-[220px]">
                <button
                    ...
                    className="flex items-center gap-2 rounded px-4 py-2 w-full transition-colors hover:bg-white/10"
                    style={{
                      background: '#001a3a',
                      border: errors.dorsal ? '2px solid #FF6B6B' : '1px solid rgba(255,255,255,0.3)',
                      fontFamily: "'Poppins', sans-serif"
                    }}>
                  {dorsalImg && (
                      <img src={dorsalImg} alt={`Dorsal ${dorsal}`} className="w-8 h-8 object-contain rounded-full" />
                  )}
                  <span className="text-sm" style={{ color: dorsal ? 'white' : errors.dorsal ? '#FF6B6B' : 'rgba(255,255,255,0.6)' }}>
                  {dorsal ? `Dorsal ${dorsal}` : 'Seleciona tu Dorsal'}
                </span>
                </button>
                {errors.dorsal && (
                    <span className="text-red-400 text-xs" style={{ fontFamily: "'Inter', sans-serif" }}>
              ...
              <div className="flex flex-col gap-1 w-[220px]">
                <div className="relative">
                  <button
                      ...
                      className="flex items-center gap-3 rounded px-4 py-2 w-full justify-between"
                      style={{
                        background: '#001a3a',
                        border: errors.pie ? '2px solid #FF6B6B' : '1px solid rgba(255,255,255,0.3)',
                        fontFamily: "'Poppins', sans-serif"
                      }}>
                  <span className="text-sm" style={{ color: pieDominante ? 'white' : errors.pie ? '#FF6B6B' : 'rgba(255,255,255,0.6)' }}>
                    {pieDominante || 'Pie Dominante'}
                  </span>
                  ...
                  {showPie && (
                      <div className="absolute bottom-full left-0 w-full rounded mb-1 z-30"
                           style={{ background: '#001a3a', border: '1px solid rgba(255,255,255,0.3)' }}>
                        {pies.map(p => (
                            <button key={p} onClick={() => { setPieDominante(p); setShowPie(false); setErrors(prev => ({ ...prev, pie: '' })); }}
            ...
            <div className="flex flex-col justify-end self-end mb-4">
              <button
                  ...
                  className="px-10 py-3 text-white text-base rounded transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{
                    background: todoCompleto ? '#003C81' : '#001a3a',
                    border: '1px solid rgba(255,255,255,0.3)',
                    fontFamily: "'Poppins', sans-serif"
                  }}
                  onMouseEnter={e => { if (todoCompleto) e.currentTarget.style.background = '#0050b3'; }}
                  onMouseLeave={e => { if (todoCompleto) e.currentTarget.style.background = '#003C81'; }}>
                Continuar
              </button>
            </div>
